...
public class SessionUtils {
	...
	private static final String INITSYSTEM_ATTRIBUTE = "initSystem";
    ...
    public static void setInitSystem(HttpServletRequest request, InitSystem initSystem) {
        request.getSession().setAttribute(INITSYSTEM_ATTRIBUTE, initSystem);
    }

    public static void removeInitSystem(HttpServletRequest request) {
		request.getSession().removeAttribute(INITSYSTEM_ATTRIBUTE);
	}

    public static InitSystem getInitSystem(HttpServletRequest request) {
		return (InitSystem) request.getSession().getAttribute(INITSYSTEM_ATTRIBUTE);
	}

	public static String getVcode(HttpServletRequest request) {
		return request.getSession().getAttribute(ImageVerifyCodeServlet.SESSION_ATT_NAME).toString();
